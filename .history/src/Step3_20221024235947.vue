<script setup>
    ...
    import usePub from './assets/usePub';
    import usePub2 from './utils/pub-use';
    const Pub = usePub2();
    const {getAssetsFile} = usePub2().getAssetsFile
    let gameList =[
      {
          ...
          v-model:show="showPopoverFlag[itemData.url]" placement="bottom" :actions="statusMap" @select="(value)=>onSelect(value,itemData)">
            <template #reference>
              <div :style="{backgroundColor:itemData.bgColor}" class="icons-wrap">
                <img :src="getAssetsFile(itemData.url)" class="main_img" alt="Vue logo" @click="()=>handleChange(itemData)"/>
              </div>
