<script setup>
    import { toRefs, ref, reactive, onBeforeMount,set } from 'vue';
    import './Step.css';
    import './Step3.css';
    ...
    const handleChange = (value) => {
      ...
      showPopoverFlag = Object.assign({},showPopoverFlag,{[value.url]:!showPopoverFlag[value.url]})
...
<template>
  <div class="step3-wrap">
      ...
      <ul class="game_list">
        ...
        <li class="item"  v-for="itemData in infoState.list" @click="()=>handleChange(itemData)">
              <div :style="{backgroundColor:itemData.bgColor}" class="icons-wrap">
                <img :src="usePub2(itemData.url)" class="main_img" alt="Vue logo"/>
              </div>
              <p class="icon-text">{{itemData.text}}{{itemData.url}}</p>
              <ul class="status-list" v-show="showPopoverFlag[itemData.url]">
                <li class="arrow"></li>
                <li class="status-logo"><img src="./assets/status0.png" /></li>
                <li v-for="elData in statusMap" class="status-item" @click="()=>onSelect(elData,itemData)">
                  <span class="status-color" :style="{background:classMap[elData.scope]}"></span>
                  {{ elData.text }}
                </li>
              </ul>
        </li>
...
.item {
  ...
  .status-item {
    ...
    position: relative;
    font-size: 16px;
    background: url('./assets//btn_bg1.png') no-repeat left top;
