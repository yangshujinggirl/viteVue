<script setup>
    ...
    const goPage=()=>{
        // const lis=ref(infoState.list);
        console.log('list',infoState.value)
       router.push({
        path: 'step4',
        query: {
          scope:scope.value,
          list:toRefs(infoState)
        },
      })
    }
    let gameList =[
    ...
    const onSelect=(selectValue,item)=>{
      ...
      const valus = infoState.value;
      const arr = valus.map((el)=> {
        if(el.url === item.url) {
...
<template>
  <div class="wrap">
      ...
      <ul class="game_list">
        <li class="item"  v-for="itemData in infoState" >
          <van-popover
          class="status-popver"
